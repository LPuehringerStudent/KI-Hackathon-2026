...
func _run() -> void:
	...
	check(not chat.get_node("Rows/DecisionsScroll/Decisions").get_child(0).disabled, "decisions remain usable while voice pending")
	chat.set_busy(false)
	chat._submit()
	check(submitted == 1 and chat.get_node("Rows/Composer/Input").text == "", "send clears composer")
	chat.set_resolved(true)
	check(chat.get_node("Rows/DecisionsScroll/Decisions").get_child(0).disabled, "resolved entity blocks duplicate decisions")
