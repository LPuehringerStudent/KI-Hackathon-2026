...
func test_closing_service_is_continuous_not_a_cliff() -> void:
	...
	var m: Dictionary = GS.compute_meters(state, data)
	# v1 carries 10 of 40 event_weight: losing its toilet costs 25 * 10/40, plus the flat closure penalty
	check(is_equal_approx(fresh.happiness - m.happiness, 6.25 + GS.CLOSED_SERVICE_PENALTY), "closing wc1 should cost 6.25 + CLOSED_SERVICE_PENALTY, got %s" % (fresh.happiness - m.happiness))
...
func test_pedestrian_street_near_venue_raises_attendance_until_reopened() -> void:
	...
	var state: Dictionary = GS.create()
	state.decisions.append(_decision("s1", "street", "carfree", 300.0))
	check(is_equal_approx(GS.compute_meters(state, data).attendance, 100.0 * (GS.ISOLATED_REACH + GS.CARFREE_REACH_BONUS)), "isolated + pedestrian bonus")
	check(is_equal_approx(GS.compute_meters(state, data).happiness, (GS.HAPPINESS_BASE + GS.FOUNTAIN_WEIGHT + GS.TOILET_WEIGHT + GS.SHADE_WEIGHT * 12.0 / GS.SHADE_FULL_CROWN_M - GS.SECURITY_SHORTFALL_CAP) - GS.CARFREE_HAPPINESS_PENALTY), "a pedestrian zone costs a little happiness")
